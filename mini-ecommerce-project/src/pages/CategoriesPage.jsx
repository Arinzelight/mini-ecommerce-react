    ...
    const getProductsForCategory = async () => {
      ...
      try {
        ...
        const validProducts = allProducts.filter(prod =>
          prod.title && prod.images && prod.images.length > 0 && prod.images[0].startsWith('http')
        );
                ...
                {isLoading ? ( // Show loading state for products
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {products.map(product => (
                      <div key={product.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:scale-105">
                        <img
                          src={product.images[0] || `https://placehold.co/400x300/FDFBF8/07484A?text=No+Image`}
                          alt={product.title}
                        ...
                        />
                        <div className="p-4">
                          <h3 className="text-lg font-semibold text-primary mb-1">{product.title}</h3>
